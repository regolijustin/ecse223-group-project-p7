...
public class TOBikeTour
{
  ...
  private TOGuide guide;

  //Helper Variables
  private boolean canSetParticipantCosts;
  private boolean canSetGuide;

  //------------------------
  // CONSTRUCTOR
  //------------------------

  public TOBikeTour(int aId, int aStartWeek, int aEndWeek, String aGuideEmail, String aGuideName, int aTotalCostForGuide, int aNrParticipants, TOGuide aGuide, TOParticipantCost... allParticipantCosts)
  {
    id = aId;
    ...
    nrParticipants = aNrParticipants;
    ...
    if (!setGuide(aGuide))
    {
      throw new RuntimeException("Unable to create TOBikeTour due to aGuide. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }
  ...
  private boolean setGuide(TOGuide aNewGuide)
  {
    boolean wasSet = false;
    if (!canSetGuide) { return false; }
    canSetGuide = false;
    if (aNewGuide != null)
    {
      guide = aNewGuide;
  ...
  public String toString()
  {
    return super.toString() + "["+
            ...
            "nrParticipants" + ":" + getNrParticipants()+ "]" + System.getProperties().getProperty("line.separator") +
